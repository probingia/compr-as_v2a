...
export const renderProductos = (productos = null, isExport = false, container = dom.listaComprasContainer) => {
    ...
    if (state.modoOrden !== 'categoria') {
        ...
        Object.keys(productosAgrupados).sort((a, b) => {
            ...
            h4.className = 'categoria-header border-bottom pb-2 mb-3';
            
            const nombreCategoriaSpan = document.createElement('span');
            nombreCategoriaSpan.textContent = categoria.nombre;
            h4.appendChild(nombreCategoriaSpan);

            if (!isExport) {
                const badge = document.createElement('span');
                badge.className = 'badge bg-secondary float-end';
                badge.textContent = productosAgrupados[categoriaId].length;
                h4.appendChild(badge);
            }

            categoriaWrapper.appendChild(h4);
...
export const renderGestion = () => {
    ...
    getCategories()
        ...
        .forEach(c => {
            ...
            const editButton = document.createElement('button');
            editButton.className = 'btn btn-sm btn-outline-primary btn-editar-categoria me-2';
            editButton.title = 'Editar';
            editButton.innerHTML = '<i class="bi bi-pencil"></i>';
            div.appendChild(editButton);

            const deleteButton = document.createElement('button');
            deleteButton.className = 'btn btn-sm btn-outline-danger btn-eliminar-categoria';
            deleteButton.title = 'Eliminar';
    ...
    getStores()
        ...
        .forEach(t => {
            ...
            const div = document.createElement('div');
            const editButton = document.createElement('button');
            editButton.className = 'btn btn-sm btn-outline-primary btn-editar-tienda me-2';
            editButton.title = 'Editar';
            editButton.innerHTML = '<i class="bi bi-pencil"></i>';
            div.appendChild(editButton);

            const deleteButton = document.createElement('button');
            deleteButton.className = 'btn btn-sm btn-outline-danger btn-eliminar-tienda';
            deleteButton.title = 'Eliminar';
            deleteButton.innerHTML = '<i class="bi bi-trash"></i>';
            div.appendChild(deleteButton);
    ...
    listaFiltrada.sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase())).forEach(p => {
        ...
        const div = document.createElement('div');
        const editButton = document.createElement('button');
        editButton.className = 'btn btn-sm btn-outline-primary btn-editar-autocompletado me-2';
        editButton.title = 'Editar';
        editButton.innerHTML = '<i class="bi bi-pencil"></i>';
        div.appendChild(editButton);

        const deleteButton = document.createElement('button');
        deleteButton.className = 'btn btn-sm btn-outline-danger btn-eliminar-autocompletado';
        deleteButton.title = 'Eliminar';
        deleteButton.innerHTML = '<i class="bi bi-trash"></i>';
        div.appendChild(deleteButton);
